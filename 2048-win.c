...
void setBufferedInput(bool enable)
{
    ...
    if (enable && !enabled)
    {
        // Enable buffered input (normal mode)
        enabled = true;
    }
    else if (!enable && enabled)
    {
        // Disable buffered input (raw mode)
        // In Windows, we'll handle this differently in the main loop
        enabled = false;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0)
        {
            ...
            printf("  test         Perform a set of internal test.\n");
            return EXIT_SUCCESS;
        }
    ...
    while (true)
    {
        if (_kbhit())
        {
            ...
            // Handle arrow keys - they come as two characters
            if (c == 0 || c == 224)
            {
            ...
            {
                drawBoard(board, scheme, score);
                Sleep(100); // 100 ms - Windows equivalent of usleep(100 * 1000)
                addRandom(board);
                drawBoard(board, scheme, score);
                if (gameEnded(board))
                {
